...
namespace OnceMi.Framework.Service.Admin
{
    public class UpLoadFilesService : BaseService<UpLoadFiles, long>, IUpLoadFilesService
    {
            ...
            , DateTime? expiredTime = null)
        {
            if (file == null)
            {
                throw new BusException(-1, "上文文件内容为空");
            }
            ...
            throw new BusException(-1, "文件上传失败");
        }
            ...
            , DateTime? expiredTime = null)
        {
            if (files == null || files.Count == 0)
            {
                throw new BusException(-1, "上传文件为空");
            }
            ...
            , DateTime? expiredTime = null)
        {
            ...
            {
                throw new BusException(-1, "文件流为空");
            }
            if (stream.Length == 0)
            {
                throw new BusException(-1, "文件流为空");
            ...
            if (!stream.CanRead)
            {
                throw new BusException(-1, "无法读取文件流");
            }
            if (string.IsNullOrEmpty(oldName))
            {
                throw new BusException(-1, "无法获取历史文件名称");
            }
            if (string.IsNullOrEmpty(path))
            {
                throw new BusException(-1, "上传路径不能为空");
            }
            ...
            {
                throw new BusException(-1, $"获取文件类型失败，文件名称为：{oldName}");
            }
            ...
            , DateTime? expiredTime = null)
        {
            ...
            {
                throw new BusException(-1, "上传文件路径不能为空");
            }
            if (!File.Exists(uploadFilePath))
            {
                throw new BusException(-1, "上传文件不存在");
            }
            ...
            , DateTime? expiredTime = null)
        {
            ...
            if (!putResult)
            {
                throw new BusException(-1, $"上传文件【{newPath}】到对象储存失败");
            }
            stream.Position = 0;
        ...
        public UploadFileInfo DecodeFileKey(string key)
        {
            ...
            {
                throw new BusException(-1, "文件密钥验证失败");
            }
