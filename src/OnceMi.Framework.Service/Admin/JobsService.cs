...
namespace OnceMi.Framework.Service.Admin
{
    public class JobsService : BaseService<Jobs, long>, IJobsService
    {
        ...
        {
            Jobs job = _redis.Get<Jobs>(AdminCacheKey.GetJobKey(id));
            if (job == null)
            {
        ...
        public async Task<IPageResponse<JobItemResponse>> Query(IJobPageRequest request)
        {
            ...
            //查询全部缓存
            string[] keys = _redis.Keys(Regex.Replace(AdminCacheKey.SystemJobKey, @"\{.*\}*", "*"));
        ...
        public async Task Update(UpdateJobRequest request)
        {
            ...
            //移除缓存
            _redis.Del(AdminCacheKey.GetJobKey(job.Id));
        }
        ...
        public async Task Delete(List<long> ids)
        {
            ...
            foreach (var item in ids)
            {
                ...
                {
                    throw new BusException(-1, $"任务[{job.Name}]未停止，请先停止任务");
                }
            }
