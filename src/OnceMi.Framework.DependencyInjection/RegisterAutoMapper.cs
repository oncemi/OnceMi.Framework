...
namespace OnceMi.Framework.DependencyInjection
{
    public static class RegisterAutoMapper
    {
        ...
        {
            //自动注入Profile
            List<Type> profileTypes = new AssemblyHelper().GetExportedTypesByBaseType(typeof(Profile));
        ...
        {
            List<Type> types = new AssemblyHelper().GetExportedTypesByAttribute(typeof(MapperFromAttribute));
        ...
        {
            List<Type> types = new AssemblyHelper().GetExportedTypesByAttribute(typeof(MapperToAttribute));
            if (types == null || types.Count == 0)
                return new List<MapperEntityConfig>();
            List<MapperEntityConfig> result = new List<MapperEntityConfig>();
            foreach (var item in types)
