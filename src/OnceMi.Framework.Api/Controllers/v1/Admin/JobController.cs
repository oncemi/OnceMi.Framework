...
namespace OnceMi.Framework.Api.Controllers.v1.Admin
{
    ...
    public class JobController : ControllerBase
    {
        ...
        [HttpGet]
        [Route("[action]")]
        public async Task<IPageResponse<JobItemResponse>> PageList([FromQuery] IJobPageRequest request)
        {
        ...
        public async Task<JobItemResponse> Post(CreateJobRequest request)
        {
            ...
            {
                throw new BusException(ResultCode.JOB_CRON_ERROR, cronOut);
            }
            ...
            {
                throw new BusException(ResultCode.JOB_UNKNOW_OPERATION_TYPE, $"不允许的操作类型：{request.RequestMethod}");
            }
        ...
        public async Task Put(UpdateJobRequest request)
        {
            ...
            if (request.IsEnabled)
            {
                ...
                if (job == null)
                {
                    throw new BusException(ResultCode.JOB_LOAD_ERROR, "更新任务数据成功，但是重新加载任务过程中出现了错误，无法获取任务数据");
                }
                await _jobSchedulerService.Add(job);
            }
        }
